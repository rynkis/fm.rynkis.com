...
$(document).ready(function () {
  //播放列表//
  $.getJSON("src/playlist.php?_=" + (new Date()).getTime(),
  function (playList) {
    songNum = playList.length
    for (var i = 0, pLen = songNum; i < pLen; i++) {
      html += '<li data-index=' + i + '>' + playList[i].name + ' - ' +
              playList[i].artist[0] + '</li>'
    }
    ...
    //播放器//
    cd_size()
    $.getJSON("src/player.php?id=" + playList[0].id + "&肏你媽哦=" +
    (new Date()).getTime(), function (mp3_info) {
...
function load_music(id,ikite) {
  $.getJSON("src/player.php?id=" + id + "&妳媽死了=" + (new Date()).getTime(),
  function (mp3_info) {
    $("#player").attr("src", mp3_info.mp3)
